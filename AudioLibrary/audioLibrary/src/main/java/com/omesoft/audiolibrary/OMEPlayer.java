...
//	private static Context context;
    ...
    public static boolean initAudioEngine(Context context) {
        if (wRefContext == null)
            wRefContext = new WeakReference<Context>(context);
        // 保证只初始化一次
     ...
     * @param flag     <li><b>BASS_SAMPLE_FLOAT</b> <br>
     *                 使用32位浮点样本数据<br>
    ...
    /**
     * 创建播放文件(sdcard or url[http开头或https])
     *
     * @param filePath
     * @param offSet   偏移量，从什么地方开始播放，0从文件头开始
     * @param length   播放长度，0为播放到文件尾
    ...
    public void setDataSource(String filePath, int offSet, int length, int flag) {
        if (filePath.startsWith("http://") || filePath.startsWith("https://")) {
            setDataScore(filePath, offSet, flag, null, null);
        } else {
    ...
    public void setDataScore(String filePath, int offSet, int flag, BASS.DOWNLOADPROC downloadCallback, Object data) {
        mHandle = BASS.BASS_StreamCreateURL(filePath, offSet, flag, downloadCallback, data);
        printError("setDataSource(" + filePath + "," + offSet + "," + flag + "," + downloadCallback + "," + data + ")");
        if (mHandle == 0) {
            release();
            return;
        }
     ...
     * 设置EQ
     *
     * @param band   频带号，用于标记每一个频带，不能重复、自己定义
     * @param center 中心频率，1HZ~freq/2HZ
     * @param gain   增益大小，-15db~15db
     * @param key    记录每个EQ的句柄的key
     * @return
     */
